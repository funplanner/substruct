...
class ContentNodeTest < ActiveSupport::TestCase
  ...
  def test_should_not_create_invalid_content_node
    cn = ContentNode.new
    assert !cn.valid?
    assert cn.errors[:name].any?
    assert cn.errors[:title].any?
    assert cn.errors[:content].any?
    # A content node must have a name, a title and a content.
    assert_equal ["can't be blank"], cn.errors[:name]
    assert_equal ["can't be blank"], cn.errors[:title]
    assert_equal ["can't be blank"], cn.errors[:content]
  end
