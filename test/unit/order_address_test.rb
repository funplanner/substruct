...
class OrderAddressTest < ActiveSupport::TestCase
  ...
  def test_should_not_create_invalid_address
    ...
    assert "#{ERROR_EMPTY} If you live in a country that doesn't have postal codes please enter '00000'.", an_address.errors[:zip].any? ## FIXME: Why are validations pushing DUPLICATE messages into errors[] ?
    assert ERROR_EMPTY, an_address.errors[:telephone].any? ## FIXME: Why are validations pushing DUPLICATE messages into errors[] ?
    assert ERROR_EMPTY, an_address.errors[:first_name].any? ## FIXME: Why are validations pushing DUPLICATE messages into errors[] ?
