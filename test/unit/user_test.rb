...
#    )
  ...
  def test_should_not_create_invalid_user
    ...
    assert an_user.errors[:login].any?
    assert_equal ["is too short (minimum is 3 characters)", "can't be blank"], an_user.errors[:login]

    # A user must have a not so long login.
    an_user.login = "my_very_very_very_very_very_very_long_login"
    assert !an_user.valid?
    assert an_user.errors[:login].any?
    assert_equal ["is too long (maximum is 40 characters)"], an_user.errors[:login]

    # A user must have an unique login.
    an_user.login = "admin"
    assert !an_user.valid?
    assert an_user.errors[:login].any?
    assert_equal ["has already been taken"], an_user.errors[:login]
    ...
    an_user.password_confirmation = ""
    assert !an_user.valid?
    assert an_user.errors[:password].any?
    assert_equal [" must be between 5 and 40 characters."], an_user.errors[:password]
    ...
    assert !an_user.valid?
    assert an_user.errors[:password].any?
    assert_equal [" and confirmation don't match."], an_user.errors[:password]
    an_user.password_confirmation = "another_password"
    assert !an_user.valid?
    assert an_user.errors[:password].any?
    assert_equal [" and confirmation don't match."], an_user.errors[:password]
